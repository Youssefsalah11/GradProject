...
def LaneStay(frame,thresh):
    ...
    curveAveragePoint,imgHist = getHistogram(wrapped_thresh_img, True, 0.8,1)
    ...
    display_IMG=display2(frame,wrapped_thresh_img,frame_withPoints,imgHist,points,wT,hT,curve,1)
...
def poly(image):
    ...
    while True:
        ...
        try:
            ...
            # # Keep the original fram
            # frame = Image.fromarray(cv2.cvtColor(frame,cv2.COLOR_BGR2RGB))
            # plt.figure(figsize=(6, 6))
            # plt.title("label")
            # plt.imshow(frame_pil)  # Display the image in grayscale
            # plt.show()
            ...
            binary_mask_image = (output.numpy()[0] * 255).astype(np.uint8)

            image1=LaneStay(image,binary_mask_image)
            ...
            #     cv2.polylines(image2_color, [pointsL], False, (255, 0, 0), 2) 
            #     cv2.polylines(image2_color, [pointsR], False, (255, 0, 0), 2) 
            ...
            #         cv2.line(image2_color, (average_x, int(height2 * 0.8)), (average_x, height2), (255, 0, 0), 2)
            ...
            #     cv2.line(image2_color, (closest_points[0], center_y), (center_x, center_y), (0, 0, 255), 2)
            #     cv2.line(image2_color, (closest_points[0]+1,center_y-10),(closest_points[0]+1,center_y+10),(0, 0, 255), 5)
            #     cv2.line(image2_color, (center_x+1,center_y-10),(center_x+1,center_y+10),(0, 0, 255), 5)
            ...
            # steering_angle = calculate_steering_angle(left_line_poly, right_line_poly, image_width)
            # font = cv2.FONT_HERSHEY_SIMPLEX
            # # Display the steering instruction
            # if steering_angle > 15.0:
            #     instruction = "Sharp Right"
            # elif 5 < steering_angle <= 15.0:
            #     instruction = "Turn Right"
            # elif 4 < steering_angle <= 5:
            #     instruction = "Slight Right"
            # elif -4 <= steering_angle <= 4:
            #     instruction = "Go Straight"
            # elif -5 <= steering_angle < -4:
            ...
            # cv2.putText(image_color, f"Steering Instruction: {instruction}", (10, 50), font, 1, (255, 255, 255), 2, cv2.LINE_AA)            
            ...
            # A value of zero means the steering should remain straight, implying the vehicle is correctly aligned with the desired path or lan
            cv2.imshow('land detection window', image1)
            cv2.waitKey(500)
            print("ana shghala2")
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                bp.set_attribute('sensor_tick', '0.03333333333')
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            # --My code---------------------------------------------------------------
            # ==============================================================================
            ld_im = np.array(image.raw_data)
            ld_im = ld_im.copy()
            ld_im = ld_im.reshape((image.height, image.width, 4))
            ld_im = ld_im[:, :, :3]
            ...
            sized_im = cv2.resize(ld_im, dsize=(640, 480))  # VGA resolution

            #     # size_im = cv2.resize(ld_im, dsize=(800, 600))  # SVGA resolution
            #     # size_im = cv2.resize(ld_im, dsize=(1028, 720))  # HD resolution
            #     # size_im = cv2.resize(ld_im, dsize=(1920, 1080))  # Full-HD resolution
            #     #################################################
                #  cv2.imshow('land detection window', poly(sized_im))
                #  cv2.waitKey(1)
            # except Exception as e:
            #     print("Error during image processing:", e)
        if self.recording:
...
if __name__ == '__main__':
    image_thread = threading.Thread(target=poly, args=(sized_im,))
    image_thread.start()
    main()
